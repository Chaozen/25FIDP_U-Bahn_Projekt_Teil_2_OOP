"""
Modul: utils.py
Beschreibung: Logik und Hilfsfunktionen (Input-Bereinigung).
"""
from streckendaten import STRECKEN_NETZ

# --- LOGIK TEIL A: Strecken-Logik ---
def berechne_fahrzeiten_ab_start():
    start_bahnhof = "A"
    ergebnis = {}
    aktuelle_haltestelle = start_bahnhof
    aktuelle_zeit = 0
    ergebnis[aktuelle_haltestelle] = aktuelle_zeit

    while STRECKEN_NETZ[aktuelle_haltestelle]:
        naechster_halt, fahrzeit = STRECKEN_NETZ[aktuelle_haltestelle][0]
        aktuelle_zeit += fahrzeit
        ergebnis[naechster_halt] = aktuelle_zeit
        aktuelle_haltestelle = naechster_halt
    return ergebnis
